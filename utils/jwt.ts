...
/**
 * Encode JWT token from header and payload
 * Note: This creates an UNSIGNED token for testing purposes
 * For production use, tokens should be signed by a server
 */
export function encodeJwt(headerJson: string, payloadJson: string, secret?: string): string {
  try {
    ...
    // If no secret provided, create unsigned token
    if (!secret) {
    ...
    return signJwt(message, secret, headerObj.alg)
  } catch (error) {
 ...
 */
async function signJwt(message: string, secret: string, algorithm: string = 'HS256'): Promise<string> {
  try {
    ...
    throw new Error('Error signing JWT: ' + (error as Error).message)
  }
}
